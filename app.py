...
def daily_visit_status_map(seller, client_ids):
    ...
    for visit in visits:
        if visit.client_id not in result:
            result[visit.client_id] = {'result': visit.result, 'visited_at': to_rivera(visit.created_at).strftime('%H:%M')}
    return result
...
def dashboard():
    ...
    route_map_clients = [{'id': c.id, 'code': c.code, 'name': c.name, 'address': c.address, 'seller': c.seller, 'day': c.day, 'latitude': c.latitude, 'longitude': c.longitude, 'order': c.optimized_order or c.original_order, 'visit_result': visit_statuses.get(c.id, {}).get('result'), 'visited_at': visit_statuses.get(c.id, {}).get('visited_at')} for c in clients]
...
def daily_route_map_api():
    ...
    for c in rows:
        ...
        payload.append({'id': c.id, 'code': c.code, 'name': c.name, 'address': c.address, 'seller': c.seller, 'day': c.day, 'latitude': c.latitude, 'longitude': c.longitude, 'order': c.optimized_order or c.original_order, 'visit_result': status.get('result'), 'visited_at': status.get('visited_at')})
