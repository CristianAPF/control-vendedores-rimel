import json, os, secrets, time, re
...
def normalize_visit_result(value):
    """Normaliza mayúsculas, tildes y espacios para colorear estados de forma robusta."""
    import unicodedata
    text = unicodedata.normalize('NFKD', str(value or '').strip().upper())
    text = ''.join(ch for ch in text if not unicodedata.combining(ch))
    return ' '.join(text.split())

def daily_visit_status_map(seller, client_ids):
    """Último resultado registrado hoy (hora Rivera) para cada cliente."""
    ids = [int(cid) for cid in client_ids if cid is not None]
    if not seller or not ids:
        return {}
    start_dt, end_dt = local_day_utc_bounds(rivera_now().date())
    visits = Visit.query.filter(Visit.seller == seller, Visit.client_id.in_(ids), Visit.created_at >= start_dt, Visit.created_at < end_dt).order_by(Visit.created_at.desc()).all()
    result = {}
    for visit in visits:
        if visit.client_id not in result:
            result[visit.client_id] = {'result': visit.result, 'result_key': normalize_visit_result(visit.result), 'visited_at': to_rivera(visit.created_at).strftime('%H:%M'), 'date': rivera_now().date().isoformat()}
    return result
...
def clean_geocode_address(address):
    ...
    value = re.sub('(?i)\\bEsq\\.?\\s*:', ' esquina ', value)
    value = re.sub('(?i)\\bApto\\.?\\s*:', ' ', value)
    value = re.sub('(?i)\\bFte\\.?\\b', ' frente a ', value)
    value = re.sub('\\s+', ' ', value).strip(' ,')
    return value
RIVERA_CENTER = (-30.905, -55.55)
RIVERA_CITY_BBOX = {'south': -31.05, 'north': -30.8, 'west': -55.72, 'east': -55.38}
...
def geocode_queries(address):
    ...
    values = [f'{cleaned}, 40000 Rivera, Rivera, Uruguay', f'{cleaned}, Rivera, Uruguay']
    parts = re.split('(?i)\\s+esquina\\s+', cleaned, maxsplit=1)
    if len(parts) == 2:
        main, cross = (parts[0].strip(' ,'), parts[1].strip(' ,'))
        values.extend([f'{main} y {cross}, 40000 Rivera, Rivera, Uruguay', f'{main} esquina {cross}, Rivera, Uruguay', f'{main}, 40000 Rivera, Rivera, Uruguay'])
    without_notes = re.sub('(?i)\\b(frente a|bis|local|apto|km)\\b.*$', '', cleaned).strip(' ,')
    if without_notes and without_notes.lower() != cleaned.lower():
        values.extend([f'{without_notes}, 40000 Rivera, Rivera, Uruguay', f'{without_notes}, Rivera, Uruguay'])
    result = []
    seen = set()
    for q in values:
        key = q.casefold()
...
def coordinate_in_rivera(lat, lon):
    ...
    try:
        ...
        return False
    return RIVERA_CITY_BBOX['south'] <= lat <= RIVERA_CITY_BBOX['north'] and RIVERA_CITY_BBOX['west'] <= lon <= RIVERA_CITY_BBOX['east']
...
def geocode_client_server(address):
    ...
    for query in geocode_queries(address):
        params = urlencode({'format': 'jsonv2', 'limit': 3, 'countrycodes': 'uy', 'accept-language': 'es', 'addressdetails': 1, 'viewbox': viewbox, 'bounded': 1, 'q': query})
        try:
...
def next_route_order(seller, day):
    current = db.session.query(db.func.max(db.func.coalesce(Client.optimized_order, Client.original_order))).filter(Client.seller == seller, Client.day == day).scalar() or 0
...
def seed_data():
    ...
    if User.query.count() == 0:
        defaults = [('gerencia', 'Gerencia RIMEL', 'manager', None, 'Rimel2026!'), ('gerson', 'Gerson', 'seller', 'Gerson', 'Gerson2026!'), ('eduardo', 'Eduardo', 'seller', 'Eduardo', 'Eduardo2026!'), ('victoria', 'Victoria', 'seller', 'Victoria', 'Victoria2026!')]
...
def dashboard():
    ...
    if u.role == 'seller' and (not day_filter):
        ...
        if weekday_idx < 6:
            day_filter = DAYS[weekday_idx]
    date_from_raw = request.args.get('date_from', '').strip()
    date_to_raw = request.args.get('date_to', '').strip()
    try:
        date_from = date.fromisoformat(date_from_raw) if date_from_raw else date.today()
    except ValueError:
    ...
    if day_filter:
        cq = cq.filter_by(day=day_filter)
    clients = cq.order_by(route_order_expression(), db.func.coalesce(Client.optimized_order, Client.original_order), Client.name).all()

    # Búsqueda rápida: para vendedores busca en toda su cartera, sin limitar por día.
    if u.role == 'seller' and client_search:
        term = f"%{client_search}%"
        search_results = (
            Client.query
            .filter_by(active=True, seller=u.seller_name)
            .filter(db.or_(
                Client.name.ilike(term),
                Client.address.ilike(term),
                Client.code.ilike(term)
            ))
            .order_by(Client.day, route_order_expression(), db.func.coalesce(Client.optimized_order, Client.original_order), Client.name)
            .limit(50)
            .all()
        )
    ...
    if u.role == 'seller':
        vq = vq.filter_by(seller=u.seller_name)
    elif seller_filter:
        vq = vq.filter_by(seller=seller_filter)
    if day_filter and client_ids:
        vq = vq.filter(Visit.client_id.in_(client_ids))
    elif day_filter and (not client_ids):
        vq = vq.filter(db.text('1=0'))
    ...
    if u.role == 'seller':
        complaints_q = complaints_q.filter_by(seller=u.seller_name)
    elif seller_filter:
        complaints_q = complaints_q.filter_by(seller=seller_filter)
    ...
    if seller_filter:
        active_sellers_q = active_sellers_q.filter(Visit.seller == seller_filter)
    active_sellers = active_sellers_q.count()
    metrics = {'planned': total, 'visits': visits, 'unique': unique, 'compliance': round(unique / total * 100, 1) if total else 0, 'orders': orders, 'conversion': round(orders / effective * 100, 1) if effective else 0, 'contact_rate': round(contacted / visits * 100, 1) if visits else 0, 'no_restock': no_restock, 'absent': absent, 'closed': closed, 'reprogrammed': reprogrammed, 'inactive': inactive, 'gps_rate': round(gps_count / visits * 100, 1) if visits else 0, 'notes_rate': round(notes_count / visits * 100, 1) if visits else 0, 'repeat_visits': repeat_visits, 'avg_daily': round(visits / period_days, 1), 'complaints': complaints_total, 'complaints_pending': complaints_pending, 'complaint_rate': round(complaints_total / visits * 100, 1) if visits else 0, 'active_sellers': active_sellers}
    ...
    if u.role == 'manager':
        for seller in sellers:
            ...
            sv = sq.all()
            su = len({v.client_id for v in sv})
            so = sum((v.result == 'PEDIDO CONCRETADO' for v in sv))
            ...
            if day_filter:
                sp = sp.filter_by(day=day_filter)
            planned = sp.count()
            latest = LocationPing.query.filter_by(seller=seller).order_by(LocationPing.created_at.desc()).first()
            seller_stats.append({'seller': seller, 'planned': planned, 'visits': len(sv), 'unique': su, 'compliance': round(su / planned * 100, 1) if planned else 0, 'orders': so, 'conversion': round(so / se * 100, 1) if se else 0, 'last_seen': latest.created_at if latest else None})
    visit_statuses = daily_visit_status_map(u.seller_name, [c.id for c in clients]) if u.role == 'seller' else {}
    route_map_clients = [{'id': c.id, 'code': c.code, 'name': c.name, 'address': c.address, 'seller': c.seller, 'day': c.day, 'latitude': c.latitude, 'longitude': c.longitude, 'order': c.optimized_order or c.original_order, 'visit_result': visit_statuses.get(c.id, {}).get('result'), 'visit_result_key': visit_statuses.get(c.id, {}).get('result_key'), 'visited_at': visit_statuses.get(c.id, {}).get('visited_at'), 'visit_date': visit_statuses.get(c.id, {}).get('date')} for c in clients]
    return render_template('dashboard.html', clients=clients, metrics=metrics, sellers=sellers, selected_seller=seller_filter if u.role == 'manager' else '', selected_day=day_filter, route_map_clients=route_map_clients, visit_statuses=visit_statuses, client_search=client_search, search_results=search_results, date_from=date_from.isoformat(), date_to=date_to.isoformat(), seller_stats=seller_stats)
...
def create_visit():
    ...
    if not client or (u.role == 'seller' and client.seller != u.seller_name):
    ...
    if complaint:
        db.session.add(Complaint(visit_id=visit.id, client_id=client.id, seller=client.seller, text=complaint, category=request.form.get('complaint_category', 'OTRA'), priority=request.form.get('complaint_priority', 'MEDIA')))
    db.session.commit()
    flash('Visita registrada y sincronizada con Gerencia.', 'success')
    return redirect(url_for('dashboard'))
...
def history():
    ...
    if date_from_raw:
        try:
            start_dt, _ = local_day_utc_bounds(date.fromisoformat(date_from_raw))
            q = q.filter(Visit.created_at >= start_dt)
        except ValueError:
            pass
    if date_to_raw:
        try:
            _, end_dt = local_day_utc_bounds(date.fromisoformat(date_to_raw))
            q = q.filter(Visit.created_at < end_dt)
        except ValueError:
            pass
    ...
    return render_template('history.html', visits=visits, sellers=sellers, selected_seller=seller, selected_result=result, date_from=date_from_raw, date_to=date_to_raw)
...
def complaints():
    ...
    return render_template('complaints.html', complaints=rows, sellers=sellers, selected_seller=seller, selected_status=status, selected_priority=priority)
...
def export_history():
    ...
    if df:
        try:
            start, _ = local_day_utc_bounds(date.fromisoformat(df))
            q = q.filter(Visit.created_at >= start)
        except ValueError:
            pass
    if dt:
        try:
            _, end = local_day_utc_bounds(date.fromisoformat(dt))
            q = q.filter(Visit.created_at < end)
        except ValueError:
            pass
...
def export_complaints():
    ...
    for c in q.order_by(Complaint.created_at.desc()).all():
        created = to_rivera(c.created_at)
        resolved = to_rivera(c.resolved_at)
...
def clients_admin():
    ...
    sellers = sorted([x[0] for x in db.session.query(Client.seller).distinct().all()])
    return render_template('clients.html', clients=rows, sellers=sellers, selected_seller=seller, selected_day=day, selected_status=status, search=search)
...
def client_create():
    ...
    c = Client(code=code, name=name, address=address, seller=seller, day=day, original_order=order, optimized_order=order, active=True)
    db.session.add(c)
    db.session.commit()
...
def route_clients_api(seller, day):
    ...
    rows = Client.query.filter_by(seller=seller, day=day, active=True).order_by(db.func.coalesce(Client.optimized_order, Client.original_order), Client.name).all()
    return jsonify([{'id': c.id, 'name': c.name, 'address': c.address, 'latitude': c.latitude, 'longitude': c.longitude, 'order': c.optimized_order or c.original_order} for c in rows])
...
def save_optimized_route():
    ...
    for c in remaining:
        pos += 1
        c.optimized_order = pos
    db.session.commit()
    return jsonify({'ok': True, 'saved': len(received_ids), 'total': len(valid)})

@app.get('/api/daily-route-map')
@login_required
def daily_route_map_api():
    u = current_user()
    day = normalize_day(request.args.get('day', ''))
    if day not in DAYS:
        return (jsonify({'error': 'Día inválido'}), 400)
    seller = u.seller_name if u.role == 'seller' else request.args.get('seller', '').strip()
    if not seller:
        return (jsonify({'error': 'Vendedor no indicado'}), 400)
    rows = Client.query.filter_by(seller=seller, day=day, active=True).order_by(db.func.coalesce(Client.optimized_order, Client.original_order), Client.name).all()
    try:
    ...
    for c in missing[:batch]:
        result = geocode_client_server(c.address)
        processed += 1
        if result:
            c.latitude, c.longitude, _source = result
            db.session.commit()
    rows = Client.query.filter_by(seller=seller, day=day, active=True).order_by(db.func.coalesce(Client.optimized_order, Client.original_order), Client.name).all()
    statuses = daily_visit_status_map(seller, [c.id for c in rows])
    payload = []
    remaining = 0
    for c in rows:
        ...
        payload.append({'id': c.id, 'code': c.code, 'name': c.name, 'address': c.address, 'seller': c.seller, 'day': c.day, 'latitude': c.latitude, 'longitude': c.longitude, 'order': c.optimized_order or c.original_order, 'visit_result': status.get('result'), 'visit_result_key': status.get('result_key'), 'visited_at': status.get('visited_at'), 'visit_date': status.get('date')})
    return jsonify({'clients': payload, 'total': len(rows), 'located': len(rows) - remaining, 'remaining': remaining, 'processed': processed})
...
def save_client_coordinate():
    ...
    c = db.session.get(Client, client_id)
    if not c or (u.role == 'seller' and c.seller != u.seller_name):
...
def users():
    if request.method == 'POST':
        ...
        if User.query.filter_by(username=username).first():
            ...
            seller = request.form.get('seller_name', '').strip() or None
            db.session.add(User(username=username, full_name=request.form['full_name'].strip(), role=role, seller_name=seller, password_hash=generate_password_hash(request.form['password']), must_change_password=False))
            db.session.commit()
...
def reset_user(uid):
    ...
    pw = request.form.get('password', 'Rimel2026!')
    if u:
...
def api_summary():
    u = current_user()
    since, _ = local_day_utc_bounds(rivera_now().date())
    q = Visit.query.filter(Visit.created_at >= since)
    if u.role == 'seller':
        q = q.filter_by(seller=u.seller_name)
    rows = q.all()
    return jsonify({'visits': len(rows), 'orders': sum((v.result == 'PEDIDO CONCRETADO' for v in rows)), 'last_update': rivera_now().isoformat()})

@app.post('/api/location')
@login_required
def save_location():
    u = current_user()
    if u.role != 'seller' or not u.seller_name:
        return (jsonify({'error': 'Solo vendedores'}), 403)
    data = request.get_json(silent=True) or {}
    try:
    ...
    def val(name):
        try:
            ...
            return None
    ping = LocationPing(seller=u.seller_name, user_id=u.id, latitude=lat, longitude=lon, accuracy=val('accuracy'), speed=val('speed'), heading=val('heading'), battery=val('battery'))
    db.session.add(ping)
    db.session.commit()
...
def api_locations():
    ...
    for seller in sellers:
        ...
        if p:
            ...
            result.append({'seller': seller, 'latitude': p.latitude, 'longitude': p.longitude, 'accuracy': p.accuracy, 'speed': p.speed, 'heading': p.heading, 'created_at': p.created_at.isoformat() + 'Z', 'local_time': to_rivera(p.created_at).strftime('%d/%m/%Y %H:%M:%S'), 'age_seconds': round(age), 'online': age <= 180})
        else:
...
def manager_dashboard_data():
    ...
    today_local = rivera_now().date()
    try:
        date_from = date.fromisoformat(request.args.get('date_from', '').strip()) if request.args.get('date_from', '').strip() else today_local
    except ValueError:
        date_from = today_local
    try:
        date_to = date.fromisoformat(request.args.get('date_to', '').strip()) if request.args.get('date_to', '').strip() else today_local
    except ValueError:
        date_to = today_local
    if date_to < date_from:
        date_from, date_to = date_to, date_from
    ...
    if day_filter:
        cq = cq.filter_by(day=day_filter)
    clients = cq.order_by(Client.seller, route_order_expression(), db.func.coalesce(Client.optimized_order, Client.original_order), Client.name).all()
    client_ids = [c.id for c in clients]
    ...
    if client_ids:
        ...
        vq = vq.filter(db.text('1=0'))
    visits = vq.order_by(Visit.created_at.asc(), Visit.id.asc()).all()

    latest_by_client = {}
    for v in visits:
        latest_by_client[v.client_id] = v

    category_meta = {
        'PEDIDO CONCRETADO': ('COMPRÓ', 'green'),
        'SIN NECESIDAD DE REPOSICION': ('SIN REPOSICIÓN', 'red'),
        'CLIENTE AUSENTE': ('AUSENTE', 'yellow'),
        'CLIENTE CERRADO': ('CERRADO', 'yellow'),
        'VISITA REPROGRAMADA': ('REPROGRAMADO', 'yellow'),
        'CLIENTE INACTIVO': ('INACTIVO', 'gray'),
        'OTRO': ('OTRO RESULTADO', ''),
        'NO VISITADO': ('NO VISITADO', 'blue'),
    }
    counts = {key: 0 for key in category_meta}
    details_all = []
    for c in clients:
        v = latest_by_client.get(c.id)
        if v:
            key = normalize_visit_result(v.result)
            if key not in category_meta:
                key = 'OTRO'
            local = to_rivera(v.created_at)
            result = v.result
            notes = v.notes or ''
            ...
            visit_time = local.strftime('%H:%M:%S') if local else ''
        else:
            key = 'NO VISITADO'
            result = 'NO VISITADO EN EL PERÍODO'
            notes = ''
            has_gps = False
            visit_date = ''
            visit_time = ''
        counts[key] += 1
        label, color = category_meta[key]
        details_all.append({
            'status_key': key, 'status_label': label, 'color': color,
            'visit_date': visit_date, 'visit_time': visit_time,
            'seller': c.seller, 'day': c.day,
            'order': c.optimized_order or c.original_order,
            'code': c.code, 'name': c.name, 'address': c.address,
            'result': result, 'notes': notes, 'has_gps': has_gps,
        })

    details = [r for r in details_all if not status_filter or r['status_key'] == status_filter]
    visited = len(clients) - counts['NO VISITADO']
    complaints_q = Complaint.query.filter(Complaint.created_at >= start_dt, Complaint.created_at < end_dt)
    if seller_filter:
        complaints_q = complaints_q.filter_by(seller=seller_filter)
    summary = {
        'planned': len(clients), 'visited': visited, 'not_visited': counts['NO VISITADO'],
        'coverage': round(visited / len(clients) * 100, 1) if clients else 0,
        'orders': counts['PEDIDO CONCRETADO'],
        'order_rate': round(counts['PEDIDO CONCRETADO'] / visited * 100, 1) if visited else 0,
        'no_restock': counts['SIN NECESIDAD DE REPOSICION'],
        'absent': counts['CLIENTE AUSENTE'], 'closed': counts['CLIENTE CERRADO'],
        'reprogrammed': counts['VISITA REPROGRAMADA'], 'inactive': counts['CLIENTE INACTIVO'],
        'other': counts['OTRO'], 'total_visits': len(visits),
        'complaints': complaints_q.count(), 'pending_complaints': complaints_q.filter_by(status='PENDIENTE').count(),
    }

    sellers = sorted([x[0] for x in db.session.query(Client.seller).distinct().all()])
    selected_sellers = [seller_filter] if seller_filter else sellers
    seller_stats = []
    for seller in selected_sellers:
        seller_rows = [r for r in details_all if r['seller'] == seller]
        seller_counts = {}
        for r in seller_rows:
            seller_counts[r['status_key']] = seller_counts.get(r['status_key'], 0) + 1
        planned = len(seller_rows)
        not_visited = seller_counts.get('NO VISITADO', 0)
        seller_stats.append({
            'seller': seller, 'planned': planned, 'visited': planned - not_visited,
            'not_visited': not_visited,
            'coverage': round((planned - not_visited) / planned * 100, 1) if planned else 0,
            'orders': seller_counts.get('PEDIDO CONCRETADO', 0),
            'no_restock': seller_counts.get('SIN NECESIDAD DE REPOSICION', 0),
            'absent': seller_counts.get('CLIENTE AUSENTE', 0),
            'closed': seller_counts.get('CLIENTE CERRADO', 0),
            'reprogrammed': seller_counts.get('VISITA REPROGRAMADA', 0),
        })

    daily_map = {}
    for v in visits:
        local = to_rivera(v.created_at)
        day_key = local.date().isoformat() if local else ''
        row = daily_map.setdefault(day_key, {'date': local.strftime('%d/%m/%Y') if local else '', 'visits': 0, 'client_ids': set(), 'orders': 0, 'no_restock': 0, 'absent': 0, 'closed': 0, 'reprogrammed': 0})
        row['visits'] += 1
        row['client_ids'].add(v.client_id)
        result_key = normalize_visit_result(v.result)
        if result_key == 'PEDIDO CONCRETADO': row['orders'] += 1
        elif result_key == 'SIN NECESIDAD DE REPOSICION': row['no_restock'] += 1
        elif result_key == 'CLIENTE AUSENTE': row['absent'] += 1
        elif result_key == 'CLIENTE CERRADO': row['closed'] += 1
        elif result_key == 'VISITA REPROGRAMADA': row['reprogrammed'] += 1
    daily_stats = []
    for key in sorted(daily_map.keys(), reverse=True):
        row = daily_map[key]
        row['unique'] = len(row.pop('client_ids'))
        daily_stats.append(row)

    chart_items = [
        ('Compraron', counts['PEDIDO CONCRETADO'], 'green'),
        ('Sin reposición', counts['SIN NECESIDAD DE REPOSICION'], 'red'),
        ('Ausentes', counts['CLIENTE AUSENTE'], 'yellow'),
        ('Cerrados', counts['CLIENTE CERRADO'], 'yellow'),
        ('Reprogramados', counts['VISITA REPROGRAMADA'], 'yellow'),
        ('No visitados', counts['NO VISITADO'], 'blue'),
        ('Inactivos', counts['CLIENTE INACTIVO'], 'gray'),
        ('Otros', counts['OTRO'], ''),
    ]
    max_value = max([item[1] for item in chart_items] + [1])
    status_chart = [{'label': label, 'value': value, 'color': color, 'percent': round(value / max_value * 100, 1)} for label, value, color in chart_items]
    status_options = [
        ('PEDIDO CONCRETADO', 'Compraron'), ('SIN NECESIDAD DE REPOSICION', 'Sin reposición'),
        ('CLIENTE AUSENTE', 'Ausentes'), ('CLIENTE CERRADO', 'Cerrados'),
        ('VISITA REPROGRAMADA', 'Reprogramados'), ('NO VISITADO', 'No visitados'),
        ('CLIENTE INACTIVO', 'Inactivos'), ('OTRO', 'Otros'),
    ]
    return {
        'summary': summary, 'details': details, 'details_all': details_all,
        'seller_stats': seller_stats, 'daily_stats': daily_stats, 'status_chart': status_chart,
        'sellers': sellers, 'days': DAYS, 'selected_seller': seller_filter,
        'selected_day': day_filter, 'selected_status': status_filter,
        'status_options': status_options, 'date_from': date_from.isoformat(), 'date_to': date_to.isoformat(),
    }
...
def manager_dashboard():
    return render_template('manager_dashboard.html', **manager_dashboard_data())


@app.get('/export/manager-dashboard.xlsx')
@manager_required
def export_manager_dashboard():
    data = manager_dashboard_data()
    wb = Workbook()
    ws = wb.active
    ws.title = 'Resumen'
    title_fill = PatternFill('solid', fgColor='0876A8')
    header_fill = PatternFill('solid', fgColor='0F766E')
    light_fill = PatternFill('solid', fgColor='E7F3F8')
    white_font = Font(color='FFFFFF', bold=True)

    ws.append(['DASHBOARD GERENCIAL RIMEL'])
    ws['A1'].font = Font(size=16, bold=True, color='FFFFFF')
    ws['A1'].fill = title_fill
    ws.merge_cells('A1:D1')
    ws.append(['Período', data['date_from'], 'a', data['date_to']])
    ws.append(['Vendedor', data['selected_seller'] or 'Todos', 'Día', data['selected_day'] or 'Todos'])
    ws.append([])
    ...
    summary_labels = [
        ('Clientes planificados', 'planned'), ('Clientes visitados', 'visited'),
        ('No visitados', 'not_visited'), ('Cobertura (%)', 'coverage'),
        ('Compraron', 'orders'), ('Tasa de compra sobre visitados (%)', 'order_rate'),
        ('Sin necesidad de reposición', 'no_restock'), ('Ausentes', 'absent'),
        ('Cerrados', 'closed'), ('Reprogramados', 'reprogrammed'),
        ('Inactivos', 'inactive'), ('Otros resultados', 'other'),
        ('Visitas totales', 'total_visits'), ('Quejas', 'complaints'),
        ('Quejas pendientes', 'pending_complaints'),
    ]
    for label, key in summary_labels:
        ws.append([label, data['summary'][key]])

    detail_ws = wb.create_sheet('Detalle clientes')
    detail_headers = ['Estado', 'Fecha última visita', 'Hora', 'Vendedor', 'Día de ruta', 'Orden', 'Código', 'Cliente', 'Dirección', 'Resultado', 'Observaciones', 'GPS']
    detail_ws.append(detail_headers)
    for r in data['details_all']:
        detail_ws.append([r['status_label'], r['visit_date'], r['visit_time'], r['seller'], r['day'], r['order'], r['code'], r['name'], r['address'], r['result'], r['notes'], 'Sí' if r['has_gps'] else 'No'])

    sellers_ws = wb.create_sheet('Por vendedor')
    seller_headers = ['Vendedor', 'Planificados', 'Visitados', 'No visitados', 'Cobertura (%)', 'Compraron', 'Sin reposición', 'Ausentes', 'Cerrados', 'Reprogramados']
    sellers_ws.append(seller_headers)
    for s in data['seller_stats']:
        sellers_ws.append([s['seller'], s['planned'], s['visited'], s['not_visited'], s['coverage'], s['orders'], s['no_restock'], s['absent'], s['closed'], s['reprogrammed']])

    daily_ws = wb.create_sheet('Actividad diaria')
    daily_headers = ['Fecha', 'Visitas', 'Clientes únicos', 'Pedidos', 'Sin reposición', 'Ausentes', 'Cerrados', 'Reprogramados']
    daily_ws.append(daily_headers)
    for d in data['daily_stats']:
        daily_ws.append([d['date'], d['visits'], d['unique'], d['orders'], d['no_restock'], d['absent'], d['closed'], d['reprogrammed']])

    for sheet in wb.worksheets:
        header_row = 1 if sheet.title != 'Resumen' else 5
        if sheet.max_row >= header_row:
            for cell in sheet[header_row]:
                cell.fill = header_fill
                cell.font = white_font
                cell.alignment = Alignment(horizontal='center')
        sheet.freeze_panes = 'A2' if sheet.title != 'Resumen' else 'A6'
        if sheet.title != 'Resumen':
            sheet.auto_filter.ref = sheet.dimensions
        for col in range(1, sheet.max_column + 1):
            max_len = max(len(str(sheet.cell(row, col).value or '')) for row in range(1, min(sheet.max_row, 1000) + 1))
            sheet.column_dimensions[get_column_letter(col)].width = min(max(max_len + 2, 11), 55)

    output = BytesIO()
    wb.save(output)
    output.seek(0)
    filename = f"dashboard_gerencial_rimel_{data['date_from']}_{data['date_to']}.xlsx"
    return send_file(output, as_attachment=True, download_name=filename, mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
