...
def clean_geocode_address(address):
    ...
    value = re.sub(r'(?i)\bEsq\.?\s*:', ' esquina ', value)
    value = re.sub(r'(?i)\bApto\.?\s*:', ' ', value)
    value = re.sub(r'(?i)\bFte\.?\b', ' frente a ', value)
...
def daily_route_map_api():
    ...
    for c in missing[:batch]:
        ...
        if result:
            ...
            db.session.commit()
    # Recargar para devolver las coordenadas recién guardadas.
    rows=Client.query.filter_by(seller=seller,day=day,active=True).order_by(
        db.func.coalesce(Client.optimized_order,Client.original_order),Client.name
    ).all()
    payload=[]
    ...
    for c in rows:
        if c.latitude is None or c.longitude is None:
            remaining+=1
        payload.append({'id':c.id,'code':c.code,'name':c.name,'address':c.address,'seller':c.seller,'day':c.day,
                        'latitude':c.latitude,'longitude':c.longitude,'order':c.optimized_order or c.original_order})
